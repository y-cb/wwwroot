...
require_once dirname(__DIR__) . '/phonemsg_sdk/api_sdk/vendor/autoload.php';
 ...
 *
 * Created on 17/10/17.
 * 短信服务API产品的程序,工程中包含了一个SmsDemo类，直接通过
 * 执行此文件即可体验服务产品API功能
 * 备注工程编码采用UTF-8
 */
class AliSms
{
    ...
    public static function getAcsClient() {
		...
		$accessKeyId = getenv('ALIYUN_AK');
		$accessKeySecret = getenv('ALIYUN_SK');
     ...
     * @return stdClass
     */
    public static function sendSms($phone,$sign='',$template='',$content) {
        ...
        if ($template){
            ...
            $request->setTemplateCode("SMS_67140958");
        }
    ...
    public static function querySendDetails() {
        ...
        if ($this->date){
            $request->setSendDate($this->date);            
        }


        // 必填，分页大小
        if ($this->total_page){
            $request->setPageSize($this->total_page);
        } else {
        ...
        if ($this->current_page){
            $request->setCurrentPage($this->current_page);            
        } else {
