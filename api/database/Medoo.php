	...
	protected function column_push($columns)
	{
		...
		foreach ($columns as $key => $value)
		{
			...
			else
			{
                ...
                {
					if ($match[1] == 'HOUR') {
                		array_push($stack,  $match[1].'(create_at)' . ' AS ' . $this->column_quote( $match[2] ));    
					} else if ($match[1] == 'SUM'){
						array_push($stack,  $match[1].'(mergecount)' . ' AS ' . $this->column_quote( $match[2] ));
					} else if($match[1] == 'SUMC') {
						array_push($stack,  'SUM'.'(count)' . ' AS ' . $this->column_quote( $match[2] ));
					}else {
