...
/**
 * @api {GET}  /api/resource-backup 资源对象导出
 * @apiName 资源对象导出
 * @apiGroup 资源对象
 *
 *
 * @apiParam {String} type 导出类型，resource代表资源对象
 *
 *	
...
/**
 * @api {POST}  /api/resource-backup 资源对象导入
 * @apiName 资源对象导入
 * @apiGroup 资源对象
 *
 *
...
class ResourceBackupController {	
	function get(){
		$type = $_GET['type'];
	...
	function post(){
		...
		fclose($handle);
		$res = MainModel::updateConfig("USER_RES_OBJ_IN", $tmpfname);
		$code = $res['code'];
		if ($code == 0) {
			echo "ok";
		} else {
