...
class ServiceGroupObjectController extends mController{	
		...
		if($param['isTransfer']){//解决穿梭框超过100个之后数据显示问题
			if($param['name_arr']){
				...
				foreach($name_arr as $key=>$value){
					...
					if(!in_array($value['sev_name'], $compare_data)){//当穿梭框选中数据不在前100个数据中
						...
						/*if($sql_data_len<100){
							...
							$tmp_data[$sql_data_len-1-$key] = $new_arr;//选中数据在100个之后，则替换前100个中的对应下标100-key数据
						}*/
