...
/**
 * @api {GET}  /api/dhcp-backup 地址对象导出
 * @apiName dhcp导出
 * @apiGroup dhcp导出
 *
 *
 * @apiParam {String} type 导出类型，addr代表dhcp
 *
 *	
...
/**
 * @api {POST}  /api/dhcp-backup 地址对象导入
 * @apiName dhcp导入
 * @apiGroup dhcp导入
 *
 *
...
class DhcpBackupController {	
	...
	function post(){
		...
		fclose($handle);
		$res = MainModel::updateConfig("DHCP_BIND_IN", $tmpfname);
		$code = $res['code'];
		if ($code == 0) {
			echo "ok";
		} else {
