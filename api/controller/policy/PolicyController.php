...
/**
 * @api {POST} /api/policy 添加一体化策略
 * @apiName 添加一体化策略
 * @apiGroup 添加一体化策略
 *
 *
 * @apiParam {Number} id  策略ID（范围：0-10000）
 ...
 * @apiParam {Number} mode  动作（1表示 PERMIT 2表示DENY）(范围：1-2，不可为空)
 ...
 * @apiParam {Number} refer_id  相关策略ID（移动策略时表示要移动到哪条策略之前|之后，插入策略时表示要插入到哪条策略之前，不可为空）
 ...
 * @apiParam {Number} id  策略ID
 * @apiParam {Number} protocol  协议类型，1表示ipv4，2表示ipv6
 * @apiParam {Number} mv_opt  移动选项（0: 移动到指定策略之前 1: 移动到指定策略之后,不可为空）
 * @apiParam {Number} mode  动作（1表示 PERMIT 2表示DENY）(范围：1-2，不可为空)
 * @apiParam {Number} enable  策略是否启用，0表示不启用，1表示启用（范围：0-1）
 * @apiParam {Number} syslog  是否开启日志，0表示不启用，1表示启用 （范围：0-1）
 * @apiParam {Number} refer_id  相关策略ID（移动策略时表示要移动到哪条策略之前|之后，插入策略时表示要插入到哪条策略之前，不可为空）
 ...
 *	{
 *		"code": "98",
 *  	"str": "策略定义在不同接口下，不能改变顺序"
 *	}
 *
 */

class PolicyController extends mController{
	public $module = 'fw_policy_table';
}
