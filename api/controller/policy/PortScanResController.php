...
class PortScanResController extends mController{
	...
	function get(){
		...
		if($param['download']==1){//导出信息
			$id = $param['id'];
			$db = new ScanUtil();
			$onedata = $db->getInfoOne('portsan_log',$id);
			...
			//var_dump($data_res);
			//exit;
			$msg='SrcIP='.$_SERVER["REMOTE_ADDR"].' UserName='.$_SESSION[CONNECTION.USERNAME].' Operate="download port scan result" ManageStyle=web Content="operation success"';
			...
			echo $data_res; 
		}elseif ($param['detail']==1) {
			$id = $param['id'];
			$db = new ScanUtil();
			$onedata = $db->getInfoOne('portsan_log',$id);
			
			$data_json = $onedata['result'];
			$alldata= json_decode($data_json,true);
			$total = count($alldata);
			...
			$db = new ScanUtil();
			$alldata = $db->getInfo('portsan_log');  //获取数据
			$total = count($alldata);
	...
	function delete(){
		...
		$id = $param[id];
		$db = new ScanUtil();
		$db->deleInfo('portsan_log',$id);
		$msg='SrcIP='.$_SERVER["REMOTE_ADDR"].' UserName='.$_SESSION[CONNECTION.USERNAME].' Operate="delete port scan result" ManageStyle=web Content="operation success"';
		WriteLog::ConfigWrite($msg);
