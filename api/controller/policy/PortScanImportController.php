...
class PortScanImportController extends mController{
	public $source = '/mnt/boot/portscan.json';
	public $in_file = '/tmp/portscan_in.json';
	public $out_file = '/tmp/portscan.json';
	...
	function get(){
		...
		$json_type['port_scan'] = $all_json_arr;
		$json = json_encode($json_type,JSON_UNESCAPED_UNICODE);
		file_put_contents($this->out_file, $json);
		
		if( !file_exists($this->out_file) || $json_type['port_scan']==NUll){
			...
			if ($param['log']!=false) {
				$msg='SrcIP='.$_SERVER["REMOTE_ADDR"].' UserName='.$_SESSION[CONNECTION.USERNAME].' Operate="export port scan configuration" ManageStyle=web Content="operation success"';
			...
			//声明下载文件的名称
			header('Content-Disposition:attachment;filename= portscan_out.txt');//声明作为附件处理和下载后文件的名称
			//获取文件内容
		...
		if (0 == $updatefile['size']) {
			...
			if (UPLOAD_ERR_OK == $updatefile['error']) {
				...
				if (!file_exists($this->in_file)) {
					...
					$in_file_arr = $in_file_arrall['port_scan'];
					...
					if($in_file_arr==NUll){
						...
						$msg='SrcIP='.$_SERVER["REMOTE_ADDR"].' UserName='.$_SESSION[CONNECTION.USERNAME].' Operate="import port scan configuration" ManageStyle=web Content="operation success"';
						WriteLog::ConfigWrite($msg);
					}
				}
	...
	function check_item($itemvalue){
		...
		elseif(isset($itemvalue['ip_net'])){
			$wstatus = 0;
		}
		elseif(isset($itemvalue['net_v6'])){
			$wstatus = 0;
		}
	...
	function check_jarrayone($arrone)
	{
		...
		$port = ['21','22','23','25','53','80','110','111','135','137','139','143','443','445','513','1143','1723','1521','1900','2179','3306','5432','5900','8000'];

		if ($arrone['normal_port']!= NULL) {
			$serv_arr = explode(",",$arrone['normal_port']);
			foreach ($serv_arr as $ser_value) {
				if(!in_array($ser_value, $port)){
					return 1;
					break;
				}
			}
		...
		if($arrone['port_items']!=NULL){
			$portcount = self::portCount($arrone['port_items']);
			if($portcount>200){
				return 1;
			}
		}
		...
		if($arrone['name']==NUll || $arrone['scan_time']==NUll ){
			return 1;
		}
		if($arrone['scan_time']!="0" ){
			if($arrone['scan_time']!="1"){
				return 1;
			}
		}
		if($arrone['normal_port']==NUll && $arrone['port_items']==NUll && $arrone['fuzzy_port']==NUll){
			return 1;
		}
	...
	function save_json($in_file_arr)
	{
		...
		foreach ($in_file_arr as $key => $value) {
			...
			//var_dump($value['name']);
				...
				if( $check == 1 || $had_add> $need_add || $need_add < 1){ //校验相关项
					break;
				}
				else{
					$had_add = $had_add + 1;
					...
					if($value['scan_time']=="1"){  //建定时任务
						...
						$cmd = ' nohup /usr/bin/php /usr/local/wwwroot/api/libs/ScanResult.php -f '.$this->crondir.$value['name'].' >/tmp/port.log &';
