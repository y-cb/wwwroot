...
class OnlineUserStatisticController extends mController {

    public $module = 'onlineuser_statistic';
    ...
    public function get() {
        ...
        //日志关联情况获取数据
        if($_GET['filter_address']) {

            $DB = new MysqlDb();

            $data = [];
            $date = date('Ymd');
            $table_map = ['email','file_transfer','instant_message','online_shopping','app_others','search_engine','social_network'];

            foreach ($table_map as $key => $item) {

                //count db
                $count = $DB->org_count($item.'_'.$date, 'id', [
                    'srcip' => $_GET['filter_address'],
                ]);
                ...
                switch ($item) {
                    ...
                    case 'app_others': {
                        $item = str_replace('app_', '', $item);
                        break;
                    }
                    case 'instant_message': {
                        $item = str_replace('instant', 'insant', $item);
                        break;
                    }
                }
                $data['log_'.$item] = $count ? intval($count) : 0;
            }
