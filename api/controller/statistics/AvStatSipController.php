...
/**
 * @api {GET}  /api/av-sip 获取病毒防护统计TOP10攻击源IP
 * @apiName 获取病毒防护统计TOP10攻击源IP
 * @apiGroup 病毒防护统计
 *
 *
 * @apiSuccess {String} name 攻击源IP名称，不可为空，ip地址取值不固定
...
class AvStatSipController extends mController {	
	function get(){
		if(file_exists('/mnt1/mysql/')) {
			...
			$data = MysqlDb::org_select($table_name, [ 'srcip', '[COUNT](cnt)'],  ["LIMIT" => 10,  "GROUP" => "srcip", "ORDER" => "cnt DESC"]);
		}else{
			...
			if (!empty($ret)){
				for($i=0;$i<count($ret['group']);$i++){
					$ret['group'][$i]['srcip'] = $ret['group'][$i]['name'];
