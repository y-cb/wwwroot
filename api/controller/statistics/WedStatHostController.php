...
class WedStatHostController extends mController {
    ...
    public function get() {
        ...
        if($this->is_cli) {
            ...
            $cache['stat_host'] = is_array($cache['stat_host']) && !empty($cache['stat_host']) ? $cache['stat_host'] : [];
            echo json_encode($cache['stat_host']);
        }
        exit;
    }
    ...
    private function cli() {
        ...
        if ($count > 0) {
            $data = MysqlDb::org_select($table_name, [ 'host','[SUMC](cnt)'],  ["LIMIT" => 10,  "GROUP" => "host", "ORDER" => "cnt DESC"]);
        } else {
            $data = MysqlDb::org_select($table_name, [ 'host','[COUNT](cnt)'],  ["LIMIT" => 10,  "GROUP" => "host", "ORDER" => "cnt DESC"]);
        }
        if(!$data){
            $data=[];
        }
