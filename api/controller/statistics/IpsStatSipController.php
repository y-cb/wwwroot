...
class IpsStatSipController extends mController {	
	function get(){
		if(file_exists('/mnt1/mysql/')) {
			...
			$table_name = 'ips_'. $day;
			//$table_name = 'ips_20171221';
			$data = MysqlDb::org_select($table_name, [ 'srcip', '[SUM](cnt)'],  ["LIMIT" => 10,  "GROUP" => "srcip", "ORDER" => "cnt DESC"]);
			/*if(!$data){
				$data=[];
			}
