    ...
    */
    ...
    private function _getServerBaseURL()
    {
        ...
        if ( empty($this->_server['base_url']) ) {
            $cas_path = '/mnt/boot/cas.json';
            if (file_exists($cas_path)){
                $str = file_get_contents($cas_path);
                $arr = json_decode($str,true);
                $cas_arr = explode(':',$arr['cas_server']);
                $head = $cas_arr[0];
            }
            $this->_server['base_url'] = $head . '://' . $this->_getServerHostname();
    ...
    */
    ...
    public function getURL()
    {
        ...
        if ( empty($this->_url) ) {
            ...
            $cas_file = '/mnt/boot/cas.json';
            if (file_exists($cas_file)) {
                $str = file_get_contents($cas_file);
                $arr = json_decode($str,true);
                $final_uri = $arr['service'];
            }
